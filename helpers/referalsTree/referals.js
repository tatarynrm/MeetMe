...
//       const childUsers = await buildTree({ tg_id: referral.referee_id });
//       currentNode.children.push(...childUsers);
//     }
...
const buildTree = async (referrerId) => {
    // Fetch user data
    const userResult = await pool.query('SELECT * FROM users WHERE tg_id = $1', [referrerId]);
    ...
    const referredUsersResult = await pool.query('SELECT * FROM referrals WHERE referrer_id = $1', [referrerId]);
