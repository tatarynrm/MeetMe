...
console.log('addressss',address);
  if (userLoc.rows <= 0) {
    ...
    const insertQuery = 'INSERT INTO users_info (city, street, street_number, lat, long) VALUES ($1, $2, $3, $4, $5)';
    const values = [city, street, streetNumber, parseFloat(lat), parseFloat(long)];
    ...
    const updateQuery = 'UPDATE users_info SET city = $1, street = $2, street_number = $3, lat = $4, long = $5 WHERE user_id = $6';
    const values = [city, street, streetNumber, parseFloat(lat), parseFloat(long), ctx.message.from.id];
